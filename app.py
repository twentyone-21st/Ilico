...
if not os.environ.get("DATA_DIR"):
    logger.warning(
        "  [AVISO] DATA_DIR no configurado — correcciones, tokens y feedback "
        "se perderán en cada redeploy de Cloud Run. Monta un volumen GCS y "
        "configura DATA_DIR=/data en las variables de entorno del servicio."
    )

_CORRECCIONES_FILE = _DATA_DIR / "correcciones_usuario.json"
_FEEDBACK_FILE     = _DATA_DIR / "feedback_correos.json"
_FEEDBACK_LOCK     = threading.Lock()
_TOKEN_FILE        = _DATA_DIR / "gmail_token.json"

# Idempotencia del webhook: historyIds ya procesados (Pub/Sub es at-least-once)
_WEBHOOK_SEEN:     set  = set()
_WEBHOOK_LOCK            = threading.Lock()
_WEBHOOK_MAX_SEEN        = 500
...
def _get_user_id(creds=None) -> str:
    """
    Devuelve el email del usuario. Lo lee de la sesión; si no está (fallo en callback),
    lo obtiene de la API y lo guarda para la siguiente petición.
    """
    uid = session.get("user_email", "")
    if not uid and creds:
        try:
            perfil = obtener_perfil_usuario(creds)
            uid = perfil.get("email", "")
            if uid:
...
def _arrancar_modelo():
    ...
    """
    global _MODELO, _ACCURACY, _METRICAS
    logger.info("  [modelo] Cargando en background...")
    try:
        m, a, metricas = entrenar_modelo(forzar=False)
        _MODELO   = m
        _ACCURACY = a
        _METRICAS = metricas
        _MODELO_LISTO.set()
...
def auth_callback():
    ...
    try:
        ...
        # Persistir token en disco para que el renovador de push pueda usarlo sin sesión activa
        try:
            _TOKEN_FILE.write_text(json.dumps(token_dict), encoding="utf-8")
        except Exception as e:
...
@app.route("/api/push/renovar", methods=["POST"])
@csrf.exempt
...
def webhook_gmail():
    ...
    try:
        ...
        if data_b64:
            ...
            # Deduplicar notificaciones repetidas (Pub/Sub garantiza at-least-once, no exactly-once)
            if history_id:
                with _WEBHOOK_LOCK:
                    if history_id in _WEBHOOK_SEEN:
                        logger.info(f"Webhook: historyId {history_id} ya procesado, ignorado")
                        return "OK", 200
                    if len(_WEBHOOK_SEEN) >= _WEBHOOK_MAX_SEEN:
                        _WEBHOOK_SEEN.clear()
                    _WEBHOOK_SEEN.add(history_id)

            if email_address and email_address in _CACHE:
                with _CACHE_LOCK:
                    for cat in _CACHE[email_address]:
                        _CACHE[email_address][cat]["ts"] = 0.0
                logger.info(f"Webhook: cache invalidado para {email_address} (historyId={history_id})")
    except Exception as e:
