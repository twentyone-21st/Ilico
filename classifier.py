...
# Incrementar al cambiar arquitectura del modelo para forzar reentrenamiento automático
_MODEL_VERSION = "2"
...
def _metricas_por_clase(y_test: list, y_pred: list) -> dict:
    """@brief Calcula precision, recall y F1 por clase (ham/spam) como porcentajes."""
    p, r, f, s = precision_recall_fscore_support(
        y_test, y_pred, labels=["ham", "spam"], zero_division=0
    )
    return {
        "ham":  {"precision": round(float(p[0]) * 100, 1), "recall": round(float(r[0]) * 100, 1),
                 "f1": round(float(f[0]) * 100, 1), "support": int(s[0])},
        "spam": {"precision": round(float(p[1]) * 100, 1), "recall": round(float(r[1]) * 100, 1),
                 "f1": round(float(f[1]) * 100, 1), "support": int(s[1])},
    }


def _construir_pipeline() -> Pipeline:
    """
    @brief Construye el pipeline scikit-learn con TF-IDF bigramas y SGDClassifier (modified_huber).
    SGDClassifier soporta partial_fit nativo para actualizaciones incrementales reales.
...
def entrenar_modelo(forzar: bool = False) -> tuple:
    ...
    @return Tupla (modelo, accuracy, metricas) donde metricas incluye precision/recall/F1 por clase.
    """
    if not forzar and MODEL_CACHE.exists():
        ...
        try:
            with open(MODEL_CACHE, "rb") as f:
                datos = pickle.load(f)
            if datos.get("version") == _MODEL_VERSION:
                return datos["modelo"], datos["accuracy"], datos.get("metricas", {})
            logger.info("  Versión de modelo desactualizada, reentrenando...")
        except Exception:
    ...
    metricas = _metricas_por_clase(y_test, y_pred)
    logger.info(
        f"  Modelo base listo — acc={acc_base*100:.1f}% | "
        f"HAM F1={metricas['ham']['f1']}% | SPAM F1={metricas['spam']['f1']}% "
        f"({len(textos_b):,} ejemplos)"
    )

    try:
        with open(MODEL_CACHE, "wb") as f:
            pickle.dump({"modelo": modelo_base, "accuracy": acc_base,
                         "metricas": metricas, "version": _MODEL_VERSION}, f)
    except Exception:
        pass
    ...
    if HF_AVAILABLE:
        ...
        def _mejorar_con_hf():
            try:
                ...
                m2.fit(Xtr, ytr)
                y2    = m2.predict(Xte)
                a2    = accuracy_score(yte, y2)
                met2  = _metricas_por_clase(yte, y2)
                logger.info(
                    f"  [HF] Modelo mejorado — acc={a2*100:.1f}% | "
                    f"HAM F1={met2['ham']['f1']}% | SPAM F1={met2['spam']['f1']}% "
                    f"({len(textos_hf):,} ejemplos)"
                )
                with open(MODEL_CACHE, "wb") as f:
                    pickle.dump({"modelo": m2, "accuracy": a2,
                                 "metricas": met2, "version": _MODEL_VERSION}, f)
            except Exception as e:
                logger.warning(f"  [HF] No se pudo mejorar el modelo: {e}")
        _t.Thread(target=_mejorar_con_hf, daemon=True).start()

    return modelo_base, acc_base, metricas
...
def actualizar_modelo_con_ejemplo(modelo, texto: str, etiqueta: str) -> bool:
    """
    @brief Actualiza el clasificador con un nuevo ejemplo sin reentrenar desde cero usando partial_fit.
    @param modelo   Pipeline entrenado (TF-IDF + SGDClassifier).
    ...
    try:
        ...
        clf_step = modelo.named_steps.get("clf") or modelo.named_steps.get("nb")
        with _MODELO_LOCK:
            clf_step.partial_fit(X, [etiqueta], classes=["ham", "spam"])
        try:
            with open(MODEL_CACHE, "rb") as f:
                datos = pickle.load(f)
            datos["modelo"] = modelo
